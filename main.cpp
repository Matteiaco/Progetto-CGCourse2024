...
// Variabili per il movimento libero
glm::vec3 cameraPos(0.0f, 1.0f, 2.0f); // Posizione iniziale della camera
glm::vec3 cameraFront(0.0f, 0.0f, -1.0f); // Direzione in avanti
glm::vec3 cameraUp(0.0f, 1.0f, 0.0f);    // Direzione "verso l'alto"
float cameraSpeed = 0.005f;               // Velocità del movimento
float yaw = -90.0f;                      // Angolo Yaw iniziale
float pitch = 0.0f;                      // Angolo Pitch iniziale
float lastX = 400, lastY = 300;          // Ultima posizione del mouse (centrale)
bool firstMouse = true;                  // Per inizializzare il mouse
...
void generateTerrainMesh(const terrain& terr, std::vector<float>& vertices, std::vector<unsigned int>& indices) {
	...
	for (int i = 0; i < height; ++i) {
		for (int j = 0; j < width; ++j) {
			...
			// Calcolo delle derivate finite
			float dx = (j > 0 && j < width - 1) ? terr.hf(i, j + 1) - terr.hf(i, j - 1) : 0.0f;
			float dz = (i > 0 && i < height - 1) ? terr.hf(i + 1, j) - terr.hf(i - 1, j) : 0.0f;
...
void generateTrackMesh(const track& trk, std::vector<float>& vertices, std::vector<unsigned int>& indices) {
	...
	for (size_t i = 0; i < trk.curbs[0].size(); ++i) {
		...
		// Calcolo della distanza tra i punti successivi per il texture mapping
		if (i > 0) {
			accumulatedLength += glm::length(left - trk.curbs[0][i - 1]);
		}

		// Calcolo della normale per il segmento
		glm::vec3 segmentDir = (i < trk.curbs[0].size() - 1) ? trk.curbs[0][i + 1] - left : left - trk.curbs[0][i - 1];
		glm::vec3 curbDir = right - left;

		glm::vec3 normal = glm::normalize(glm::cross(segmentDir, curbDir));

		// Assicurati che la normale punti verso l'alto
		if (normal.y < 0) {
			normal = -normal;
		}
...
void resetCamera() {
	if (controlMode == 0) {
		// Reset per la trackball
		tb.reset();
		tb.set_center_radius(glm::vec3(0, 0, 0), 1.f);
	}
	else if (controlMode == 1) {
		// Reset per il movimento WASD
		cameraPos = glm::vec3(0.0f, 1.0f, 2.0f);  // Posizione iniziale
		cameraFront = glm::vec3(0.0f, 0.0f, -1.0f); // Direzione iniziale
		yaw = -90.0f; // Reset angolo Yaw
		pitch = 0.0f; // Reset angolo Pitch
	}
}
...
void gui_setup() {
	...
	if (ImGui::BeginMenu("Control Mode")) {
		if (ImGui::Selectable("Trackball", controlMode == 0)) controlMode = 0, resetCamera();
		if (ImGui::Selectable("Free Drone", controlMode == 1)) controlMode = 1, resetCamera();
		ImGui::EndMenu();
	}
	ImGui::EndMainMenuBar();
}

// Callback del mouse unificata per trackball e free drone
void unifiedMouseCallback(GLFWwindow* window, double xpos, double ypos) {
	if (controlMode == 0) {
		// Modalità Trackball
		...
		float yoffset = lastY - ypos; // Invertito per le coordinate dello schermo
		lastX = xpos;
		lastY = ypos;

		float sensitivity = 0.1f; // Sensibilità del mouse
		xoffset *= sensitivity;
		yoffset *= sensitivity;
		...
		// Limita il pitch per evitare distorsioni
		if (pitch > 89.0f)
			pitch = 89.0f;
		if (pitch < -89.0f)
			pitch = -89.0f;
...
{
	// Esegui solo se la camera selezionata è quella che guarda tutta la scena e il tipo di controllo è la trackball
	if (selected_camera == 0 && controlMode == 0) {
...
int main(int argc, char** argv)
{
	...
	glfwSetCursorPosCallback(window, unifiedMouseCallback); 
	...
	r_track.add_vertex_attribute(track_vertices.data(), track_vertices.size(), 0, 3, sizeof(float) * 8, 0); // Posizione
	r_track.add_vertex_attribute(track_vertices.data(), track_vertices.size(), 1, 2, sizeof(float) * 8, 3 * sizeof(float)); // Coordinate texture (u, v)
	r_track.add_vertex_attribute(track_vertices.data(), track_vertices.size(), 2, 3, sizeof(float) * 8, sizeof(float) * 5); // Normale
	...
	// Passa le posizioni e le coordinate texture al vertex shader
	r_terrain.add_vertex_attribute(ter_vertices.data(), ter_vertices.size(), 0, 3, sizeof(float) * 8, 0); // Posizione
	r_terrain.add_vertex_attribute(ter_vertices.data(), ter_vertices.size(), 1, 2, sizeof(float) * 8, sizeof(float) * 3); // Coordinate texture
	r_terrain.add_vertex_attribute(ter_vertices.data(), ter_vertices.size(), 2, 3, sizeof(float) * 8, sizeof(float) * 5); // Normale
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (selected_camera == 1 && selected_camera != previous_selected_camera){
			controlMode = 0;
			stack.push();
			stack.mult(glm::translate(glm::mat4(1.f), glm::vec3(0, 2, 0))); // Alza la visuale da terra
			glm::mat4 cam_frame = stack.m() * camera_frame;
			glm::vec3 camera_position = glm::vec3(cam_frame[3]); // Posizione della telecamera
			view = glm::inverse(cam_frame);
			proj = glm::perspective(glm::radians(60.f), 1.f, 2.0f, 100.f);
		...
		else if (selected_camera == 0 && selected_camera != previous_selected_camera && controlMode == 0){
			tb.reset();
			// Modalità Trackball
			view = glm::lookAt(glm::vec3(tb.matrix() * Vdir), glm::vec3(0.f, 0.f, 0.f), glm::vec3(0.f, 1.f, 0.f));
		...
		else if (selected_camera == 0 && controlMode == 1) {
			// Modalità WASD
